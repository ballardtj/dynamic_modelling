...
alpha = 0.3                   #rate at which effort returns to baseline
beta = seq(0.1, 2.1, by = 0.4)    #sensitivity of effort adjustment to change in gpd

skill_0 = 0.1                 #initial skill level
...
### nitialize variables that serve as inputs to the simulation ###

episode = rep(1:n_episodes, each = n_timesteps)
time = rep(1:n_timesteps, times = n_episodes)
practice = 1:n_total

output_list = list() #this is an object that will be used for storing the results of individual simulations

### run unique simulation for each beta value ###
...
for (p in 1:length(beta)) {
  ...
  for (i in 1:n_total) {
    ...
    if (time[i] > 1) {
      ...
      change_in_effort = alpha * (effort_bl - effort[i - 1]) + beta[p] *
        (gpd[i] - gpd[i - 1])
  ...
  output_list[[p]] = tibble(episode, time, practice, gpd, goal, effort, skill, performance) %>%
...
### combine results from individual simulations into a single data frame,
### relabelling variable names so that they appear as capitalized in the figure

output = bind_rows(output_list) %>%
  mutate(variable = factor(variable,levels=c('effort','skill','performance','goal'),
                           labels=c('Effort','Skill','Performance','Goal')))

### graph simulation results ###

ggplot(output) +
  geom_line(aes(x = time, y = level, color = factor(beta))) +
  facet_grid(variable ~ episode , scale = "free") +
  labs(x="Time",y="Level",color=expression(beta))

ggsave(file="figures/sensitivity_analysis.pdf",height=6,width=8)
