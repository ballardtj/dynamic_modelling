...
#
#
# Reciprocity—A helps (or harms) B, and B in turn helps (or harms) A

# Transitivity is the phenomenon that individuals generally share their friends’
# opinions of other people (Louch, 2000). Imagine a triad of people—A, B, and C. If A
# and B are friends, and A likes (or dislikes) C, then B should also like (or dislike) C.
...
Coeffs_Cond = matrix(rep(NA, length(numplay)*numrounds), numrounds, length(numplay))

for(TeamN in 1:length(numplay)){

  Coeffs = matrix(rep(NA, numsims*numrounds), numsims, numrounds)

  #loop for number of generations
  for(sim in 1:numsims){
    #setpoint values, with zeroes on the diagonal.
    Probx = (1-diag(1,numplay[TeamN]))*setpoint

    #Payoff Matrix: Payx = total payoff matrix from interactions of coop and
    #defecting, initialize matrix as all zeroes
    Payx = rep(0,numplay[TeamN])
    ...
    for(rounds in 1:numrounds){
      ...
      #ceiling rounds up or rounds down
      n = ceiling(runif(1)*numplay[TeamN])
      m = ceiling(runif(1)*(numplay[TeamN]-1))

      #Because you cannot play with oneself, if the row and column equal
      #each other, we add 1 to m. The m function above is numplay[TeamN]-1 because
      ...
      ########################################
      ########################################
      ##so the above just randomly samples two neighbours and determines
      ##whether or not they will interact with each other
      ########################################
      ########################################
      ...
      if (ProbInt > runif(1)) {
        ...
        ########################################
        ########################################
        ##the below just has the two neighbours randomly sample numbers
        ##the randomly samples number determines if they coop or defect
        ##the A parameter adjusts the probability of coop
        ##determine new value of the payoff matrix
        ########################################
        ########################################
      ...
      Numerator=sum(diag(matrix.power(ClusterMatrix, 3)))
      Denominator=sum(sum(matrix.power(ClusterMatrix, 2)))-sum(diag(matrix.power(ClusterMatrix, 2)))
      ...
      Coeffs[sim, rounds] = ClusterCoeff

    }
    coeff_dat = colMeans(Coeffs)
  }

  Coeffs_Cond[, TeamN] = coeff_dat
...
save(data,file="data/raw/us_and_them.RData")

#save.image(file = "capture.RData")
...
Coeffs_Cond = data.frame(Coeffs_Cond)
colnames(Coeffs_Cond) = c("N=10", "N=40", "N=70", "N=100")
Coeffs_Cond$iter = 1:5000
Coeffs_Cond = melt(Coeffs_Cond, id = "iter")

ggplot(data = Coeffs_Cond, aes(x = iter, y = value, group = variable)) + geom_line(aes(col = variable)) + theme_bw()
