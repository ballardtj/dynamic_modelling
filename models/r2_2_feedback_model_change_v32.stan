//version 32 - like version 25 except goal updates only based on previous performance
  ...
  for(i in 1:Ntotal){
    ...
    if(time[i] == 1){
      ...
      if(trial[i] > 1){
        ...
        predicted_change_in_goal[global_trial_number[i]] = theta*predicted_performance[i-1] + lambda;
