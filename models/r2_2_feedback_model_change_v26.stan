//version 26 - like version 25 but with a linear effort function
...
transformed parameters {
  ...
  for(i in 1:Ntotal){
    ...
    predicted_effort[i] = effort_intercept + effort_slope*gpd[i];
...
model {
  ...
  effort_intercept ~ normal(0,5);
  effort_slope ~ normal(0,5);
...
generated quantities {
  ...
  if(time[i] == 1){
    ...
    if(time[i] > 1){
      ...
      log_lik[i] = normal_lpdf(effort | predicted_effort[i],sigma_effort) +
