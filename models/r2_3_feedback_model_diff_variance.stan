...
parameters {
 ...
 // real g_beta;
  real<lower=0> sigma1[5];         //initialize single sigma parameter for entire sample and set lower bound at 0.
  real<lower=0> sigma2[5];         //initialize single sigma parameter for entire sample and set lower bound at 0.
  //real<lower=0> sigma3;
  //real<lower=0> sigma3;
}
...
model {
 ...
 // sigma3 ~ normal(0,1);
  ...
  for(i in 1:Ntotal){
      ...
      effort[i] ~ normal(predicted_effort[i],sigma1[time[i]]);
      score[i] ~ normal(predicted_score[i],sigma2[time[i]]);
