//version 6 - same as version 5, but includes effort intercept
  ...
  for(i in 1:Ntotal){
    ...
    if(time[i]==1){
      if(trial[i]==1){
        ...
        predicted_effort[i] = eff_int + eff_0*alpha + gain11*predicted_goal[global_trial_number[i]] +
      ...
      if(trial[i]>1){
        ...
        predicted_effort[i] = eff_int +  predicted_effort[i-1]*alpha + gain11*predicted_goal[global_trial_number[i]] +
                              gain12*predicted_ability[i]*predicted_goal[global_trial_number[i]];
    ...
    if(time[i]>1){
      ...
      predicted_effort[i] = eff_int + predicted_effort[i-1]*alpha + gain11*(predicted_goal[global_trial_number[i]] - predicted_score[i-1])  +
