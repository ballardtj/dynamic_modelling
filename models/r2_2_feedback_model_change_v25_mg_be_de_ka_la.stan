//version 25 - like version 25 mg except only beta, kappa, delta, and lambda are allowed to very across groups
  ...
  for(i in 1:Ntotal){
    ...
    if(time[i] == 1){
      ...
      if(trial[i] > 1){
        ...
        predicted_change_in_goal[global_trial_number[i]] = theta*(predicted_performance[i-1]-predicted_goal[global_trial_number[i]-1]) + lambda[condition[i]];
    ...
    if(time[i] > 1 ){
      ...
      predicted_change_in_effort[i] = alpha*(effort_baseline  - predicted_effort[i-1]) + beta[condition[i]]*(gpd[i]-gpd[i-1]);
    ...
    predicted_skill[i] = skill_max - (skill_max-skill_min)*exp(-delta[condition[i]]*practice[i]);

    // ##### CALCULATE PERFORMANCE #####

    //predicted change in performance is calculated according to Equation 4 ## NEED TO UPDATE THIS!
    predicted_change_in_performance[i] = predicted_skill[i] / (1 + exp(-kappa[condition[i]]*(predicted_effort[i] - gamma  ) ));
...
model {
  ...
  for(i in 1:Ntotal){
    ...
    if( time[i] > 1 ){
       ...
       change_in_performance ~ normal(predicted_change_in_performance[i],sigma_performance_change);
