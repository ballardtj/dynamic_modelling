...
parameters {
  ...
  real<lower=0,upper=10> effort_baseline[2];
  real beta[2];
  real<lower=0,upper=10> effort_0[2];

  //paramaeter relating to skill variable
  real<lower=0.05,upper=1> delta[2];
  real<lower=0> skill_change[2]; //difference between skill_max and skill_min;
  real<lower=0> skill_min[2];
  ...
  real lambda[2];
  real<lower=0,upper=10> goal_0[2];

  //parmaters for measurement model
  real<lower=0> sigma_effort_0[2];
  real<lower=0> sigma_effort_change[2];
  real<lower=0> sigma_performance_change[2];
  real<lower=0> sigma_goal_0[2];
  real<lower=0> sigma_goal_change[2];
}
...
transformed parameters {
  ...
  for(c in 1:2){
    skill_max[c] = skill_min[c] + skill_change[c];
  }
  ...
  for(i in 1:Ntotal){
    ...
    if(time[i] == 1){
      if(trial[i] == 1){
        ...
        predicted_goal[global_trial_number[i]] = goal_0[condition[i]];
      }

      if(trial[i] > 1){
        //if not the first trial, the change in goal level is calculated according to Equation 2.
        predicted_change_in_goal[global_trial_number[i]] = theta[condition[i]]*(predicted_performance[i-1]-predicted_goal[global_trial_number[i]-1]) + lambda[condition[i]];
    ...
    if(time[i] == 1){
      ...
      predicted_effort[i] = effort_0[condition[i]];
    }
    ...
    if(time[i] > 1 ){
      ...
      predicted_change_in_effort[i] = alpha[condition[i]]*(effort_baseline[condition[i]]  - predicted_effort[i-1]) + beta[condition[i]]*(gpd[i]-gpd[i-1]);
    ...
    predicted_skill[i] = skill_max[condition[i]] - (skill_max[condition[i]]-skill_min[condition[i]])*exp(-delta[condition[i]]*practice[i]);

    // ##### CALCULATE PERFORMANCE #####

    //predicted change in performance is calculated according to Equation 4 ## NEED TO UPDATE THIS!
    predicted_change_in_performance[i] = predicted_skill[i] / (1 + exp(-kappa[condition[i]]*(predicted_effort[i] - gamma[condition[i]]  ) ));
  ...
  for(i in 1:Ntotal){
    ...
    if( time[i] == 1){
      ...
      if(trial[i]==1){
        ...
        goal[global_trial_number[i]] ~ normal(predicted_goal[global_trial_number[i]],sigma_goal_0[condition[i]]);
      }

      if(trial[i]>1){

        change_in_goal = goal[global_trial_number[i]]-goal[global_trial_number[i]-1];
        change_in_goal ~ normal(predicted_change_in_goal[global_trial_number[i]],sigma_goal_change[condition[i]]);
    ...
    if( time[i] == 1 ){
      ...
      effort[i] ~ normal(predicted_effort[i],sigma_effort_0[condition[i]]);
      performance[i] ~ normal(predicted_performance[i],sigma_performance_change[condition[i]]);
    }

    if( time[i] > 1 ){

       change_in_effort = effort[i]-effort[i-1];
       change_in_effort ~ normal(predicted_change_in_effort[i],sigma_effort_change[condition[i]]);

       change_in_performance = performance[i]-performance[i-1];
       change_in_performance ~ normal(predicted_change_in_performance[i],sigma_performance_change[condition[i]]);
