//version 19 - like version 17 except refactors so that equations are functions and parameters have correct names
...
transformed parameters {
  ...
  for(i in 1:Ntotal){
    ...
    if(time[i] > 1 ){
      ...
      predicted_change_in_effort[i] = alpha + beta*predicted_effort[i-1] + gamma*(gpd[i]-gpd[i-1]);
    ...
    predicted_change_in_performance[i] = (gain23 + gain22*predicted_skill[i]) / (1 + exp(-(gain20 + gain24*predicted_skill[i] + gain21*predicted_effort[i]  ) ));
