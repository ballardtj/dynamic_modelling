//version 23 - like version 22 except performance assymptote is just skill and upper bound of learning curve is free
...
transformed parameters {
  ...
  for(i in 1:Ntotal){
    ...
    predicted_skill[i] = skill_max - skill_max*exp(-delta*practice[i]);

    // ##### CALCULATE PERFORMANCE #####

    //predicted change in performance is calculated according to Equation 4 ## NEED TO UPDATE THIS!
    predicted_change_in_performance[i] = predicted_skill[i] / (1 + exp(-(gain20 + gain21*predicted_effort[i]  ) ));
